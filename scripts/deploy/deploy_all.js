// shillingplus/scripts/deploy/deploy_all.js
...
async function main() {
  ...
  const VESTING_START_TIME = process.env.VESTING_START_TIME || Math.floor(Date.now() / 1000) + 3600;
  ...
  await bankIntegratedSHP.initialize(
    ...
    '0xMockBankAdapter', // Assuming a mock bank adapter address
    KSH_ORACLE,
    ETH_ORACLE,
    deployer.address
  );
